...
- [ ] **7.6.2 batch — freshness follow-ups (ratified 2026-09-01, from the phone's first live day)**: (1) suppress the post-restore ECHO push — the restore's own push is followed ~60s later by a redundant debounced push (new gen, ~17MB + 3 writes wasted, and the phone banners "6:01 newer than 6:01"); one ref + a ~5s settling-window guard in the device-state effect. (2) Gen identity IN THE DISPLAY (Ron's call, pre-launch): banner shows short copy-ids — "from 6:01 PM (…e0af) — a newer one from 6:01 PM (…9af6)"; Help&About "Library as of" gains the same suffix; console logs full ids at every freshness check. Revisit suffix-for-civilians at the Cloud-Sync copy pass. (3) Seconds-precision in banner times when both stamps share a minute (belt).
- [ ] **Backup completion feedback** (Ron, 2026-09-01; ratified for 7.6.2) — Save Backup is silent today; Ron's bridge workflow (backup local → restore prod) had him squinting at Chrome's downloads bubble. Fix: progress narration during serialization (~47MB takes real seconds; reuse restore's phase machinery) + a RICH completion toast — "Backup saved — 3,136 books, 46.7 MB → readerwrangler-backup-2026-09-01.json" (filename+count = the confidence). Deliberately NOT a modal (restore's modal is earned — the world changed; backup is fire-and-continue). Toast history makes the receipt durable. Copy note: app can only confirm hand-off to the browser; "saved" is right anyway.
...
- [ ] **Folder double-store consolidation (audit F1) + multi-tab hardening** — folders still live in FOLDERS_KEY + the blob (blob wins on load; consolidate to the single guarded source with a load reorder). Multi-tab: Web-Locks read-only second tab + reload-then-promote + live/freeze viewer, designed in docs/design/MULTI-INSTANCE.md §4. Related cleanups from the 2026-08-30 audit: restore still writes the zombie BOOKLISTS_KEY; STATUS_KEY writer is mount-unguarded (cosmetic); `rw_folders` (41KB) is an orphan key no code touches — inspect before deleting; prod holds a few dead-channel legacy device-state singles (wipe or let them TTL).

...
- [ ] **Left-pane section-header control order — make consistent** — the **+ (add)** and **collapse/expand toggle** sit in OPPOSITE orders between BOOK LISTS and FOLDERS, training conflicting muscle memory (Ron keeps hitting the wrong one). **DECIDED: move the collapse/expand chevron to the LEFT of the section label** (standard accordion pattern) for every section — Searches / Book Lists / Folders — leaving only the action buttons (+, sort ⇅) trailing on the right. One consistent, conventional layout; no ambiguity. (Ron, 6.14.0 testing.)

...
- [ ] **Inbox-scope vs Inbox-view consistency (C — deferred, NOT a forced invariant)** — the Inbox VIEW shows explicit `__inbox__` membership while Auto-Organize's scope means "unfiled (in no other folder)"; they diverge for a book that's in the Inbox AND a real folder. 6.16.0 handles the confusion gracefully (surface "already filed" + Remove-from-source) instead of forbidding the state. Optional future: align the two notions of "Inbox" so they can't disagree — WITHOUT making "the Inbox can't co-hold a filed book" a hard rule (Ron: don't force a workflow on a deliberately flexible tool).

...
- [ ] **Minimum latency floor on Amazon API calls** — ride-along while refactor #1 has the fetcher hood open. Today's politeness is emergent (the code admits it: `FETCH_DELAY_MS = 0 // network RTT ~400ms provides natural throttling`) — if Amazon ships a faster path our request rate silently multiplies. Named constant per fetcher (`MIN_REQUEST_INTERVAL_MS`) set at/just below observed RTT + light jitter; sleep only the shortfall. No-op today by construction; governs Amazon call sites only (relay is ours and already rate-limited). (External review item 4 — adopted as-is.)
...
- [ ] **readMailbox: skip absorbed runs BEFORE downloading** — today it downloads + decrypts every complete run, then filters against the generation's absorbed-runs list. The list and the key names (which carry run ids) are both known before any letter GET, so absorbed-but-not-yet-expired letters (e.g. small wishlist letters riding the 90d TTL) get pointlessly re-fetched on every import. Few-line fix: pass the absorbed set into readMailbox (or filter at call sites) and skip matching run ids pre-download. Harmless at solo scale (reads are the abundant quota) but the right shape, and it makes the code match RELAY-FLOWS' stated semantics ("keep only runs not in the ledger" / "readers, skip their letters"). (Surfaced by Ron's mailbox-mechanics question, 2026-08-26.)
...
- [ ] **Scope the Phase-4 price sweep to buyable books** (wishlist/samples/unowned) **+ owned books with an active price goal** — cuts ~104 batched calls/fetch to ~10–12; prices on owned-no-goal books answer a question nobody asks. Keep the whole wishlist fresh (goals-only overshoots — wishlist browsing/sorting by price is a real behavior).
- [ ] **Price staleness display**: prices older than 24h render dimmed/italic (named global constant, e.g. `PRICE_STALE_MS`; default 24h — sale prices can turn in a day; promote to user setting only if the constant annoys). Hover tooltip shows exact "as of ‹date›" (`priceAsOf` already stored). Book-detail dialog prints "as of ‹date›" inline next to the price. Data Status dialog gains a "Prices: last swept ‹date›" line. NO new column (price-date sort/filter serves no task), NO header banner (a single global date is approximately wrong by construction — coverage varies per fetch and scoping freezes owned prices deliberately).
- [ ] **"Price goal kept — clear it?" nudge** on wishlist→owned upgrades (goal preservation is deliberate since v5; the missing bit is surfacing it beyond the "now owned" toast).
- [ ] **Format watch (LOW priority)** — `formatWatch: 'kindle'` on print-edition placeholders; slow-lane detection of a Kindle edition appearing; on hit, OFFER (never auto) a swap to the new ASIN carrying folder/Book List/tags/price goal (cousin of the wishlist→owned upgrade). KEY VERIFY first: does the batch enrichment API expose sibling editions for a print ASIN, or does detection need per-book product-page fetches? LOW because the live cases are mostly covered: Amazon author-follow notifies on new-format releases (Heinlein), and for withdrawn-Kindle books already recorded as Kindle, the price REAPPEARING in the normal price sweep is itself the availability signal (The Destroyer's 18 withdrawn titles) — the only uncovered gap is print-placeholder→Kindle detection. (External review item 6.) (ratings/review counts — frozen at first fetch today, which is fine for owned books but mildly stale for purchase decisions). Monthly-ish or every-Nth-fetch, reusing item-2's scoped list + batch calls. 7.0 letters make it interruption-safe: results go out as a run; tab closes, so be it — nothing torn, resumes next time. NOT the per-book adaptive-TTL machinery from the external review (a beautiful solution to a scale problem we don't have). Likely shared mechanism with a future format-watch (review item 6).

...
- [ ] **Toast history** (Ron, 2026-09-01; ratified) — toasts carry real information and evaporate (three misses in one session: fetcher's "Already on your wishlist", the ghost-undo toast, sync toasts). Small 🕐/bell by the status bar → scrollable popover of the session's last ~50 toasts with timestamps, in-memory ring buffer inside showToast (VS Code notification-bell convention). Build right after 7.6.0.
- [ ] **Import-removed books can leave ghost undo entries** (2026-09-01 sweep, LOW) — a relay import absorbing another device's tombstone removes a book; older undo entries naming it would ghost-no-op. Same class as the Empty-Trash ghost-undo (FIXED alpha.21: permanent delete purges mentions); rare enough to log, not clear-on-import.
...
- [ ] **Undo stack clears mid-session unexpectedly (D)** — twice (2026-08-07/08) the undo stack emptied with no user undo/redo, blocking recovery from a bad move. **LIKELY SOLVED 2026-09-01**: the sweep for non-undoable-op protections proved NO code path clears the stack — it is in-memory only, so ANY page reload (incl. tab discard/restore) empties it silently. Almost certainly a reload sat between the action and the Ctrl+Z both times. Close after one more sighting confirms (or persist the stack someday if it matters).
...
- [ ] **Stamp the restore push** — performRestore pushes the backup JSON verbatim, so its device-state payload carries no `organization.savedAt`; a phone pulling it guard-skips and shows the pre-restore library until the next ordinary stamped push. Fix: inject the stamp at push time. (Found live 2026-08-30 while verifying the guest guard.)
- [x] ~~**DIAGNOSED**~~ → **RESOLVED in 7.6.0-alpha.6 (guest guard + deny-list cache), verified live 2026-08-30 both directions** (unstamped payload → 🛡️ skip, desktop order survived; stamped payload → ✅ applied, order STILL survived). Full story: docs/design/MULTI-INSTANCE.md. Original diagnosis: **DIAGNOSED 2026-08-30 (live forensics with Ron): the manual-order scramble.** Quota EXONERATED (217KB total, probe passes); integrity check EXONERATED (spreads preserve fields). Actual findings: (1) Ron's root order existed ONLY as implicit array order — NO sortIndex on any root folder (backups/restores carried the order as array position); (2) write vector = a SECOND localhost desktop tab (his mobile-emulation tab, chip-swapped to desktop "a time or 2") — every desktop boot/change rewrites FOLDERS_KEY+blob from its own memory, localStorage is last-writer-wins with no cross-tab notification, so a tab holding the creation-order array silently reverted the stores; display fell back to raw array order (Jim-Butcher-first). VACCINE APPLIED: Move-to-Top gesture renumbered ALL roots (wave-A canonical reorder) — order is now explicit sortIndex in both stores, array rewrites can no longer scramble the display. REMAINING HARDENING (this item's future work): F1 consolidation (blob-only + load reorder) + multi-tab write-guard (storage-event listener or second-tab detection) — a stale tab can still revert book moves/list edits, folders were just the most fragile. Also: `rw_folders` (41KB) is an orphan localStorage key NO code reads or writes — archaeology, inspect before deleting.
- [ ] (superseded context) **Folder double-store (audit F1) — PROMOTED 2026-08-30**: prime suspect for the manual-order scramble (2nd sighting, correlated with fresh-code loads). Load path: FOLDERS_KEY seeds, blob OVERRIDES when books load (readerwrangler.js ~3436/~3529); FOLDERS_KEY writer is UNGUARDED (mount-stamps), blob writer is guarded but SWALLOWS quota failures silently (~3597) → a silently-stale blob time-travels the order on next reload AND immediately re-saves the old order everywhere (destroys the evidence — why the ghost is unreproducible after the fact). Diagnostic SNAP/CHECK console snippets given to Ron 2026-08-30; cheapest tell = "Could not auto-save organization" in console, or SNAP's blobSavedAt hours stale. Consolidate to the single guarded source (blob-only + load reorder) once diagnosed. Related cleanups found in the 2026-08-30 double-store audit: restore still WRITES the zombie BOOKLISTS_KEY (~5779; read-once migration key — retire the write); STATUS_KEY writer is mount-unguarded (cosmetic stakes); EXPLORER_KEY writer guard added in 7.6.0-alpha.5.
...
- [ ] **Mobile honesty mini-wave** (diagnosed with Ron 2026-08-30; = Wave E of feature/folder-ordering, attack immediately). Three items, one theme — surfaces that quietly disagree breed hour-long mysteries:
  1. **Server indicator** (the installed-PWA-was-dev hunt) — Help/About always names the server explicitly (readerwrangler.com / Dev / Localhost); non-prod additionally wears a small DEV/LOCAL header chip (mobile twin of the 7.4.0 desktop LOCAL badge). Derive from the page's own address; installed apps inherit it, which is the whole point.
  2. **Date Added: wire + sort double bug — FIXED alpha.3/alpha.4** (incl. desktop's same mislabeling + universal author→series→position→title→ASIN tiebreak on both surfaces). REJECTED 2026-08-30: switching wishlist adds to epoch timestamps — sub-day wishlist timestamps would encode SCRAPE SEQUENCE and defeat the reading-order tiebreak; date-only + tiebreak is the better behavior, and old backups force dual-format parsing forever anyway (Ron: intra-day A-B-C order is "the lesser of 2 sins").
  2b. (superseded detail) **Date Added: wire + sort double bug** — device-state payload sends NO dateAdded field (readerwrangler.js ~4983), so mobile fabricates it from acquisitionDate (mobile.js:44) = empty for wishlist; AND mobile's "Date Added" sort key actually computes `acquired || dateAdded` (mobile.js:244) — acquisition wins even under the Date Added label. Net: ~135 wishlist books pool invisibly at the bottom of mobile All Books ("defensible logically, wrong practically — a sort whose bottom items are invisible-in-effect hides data"). Fix: send dateAdded; mobile sorts `dateAdded || acquired` (fallback for stale payloads); All Books shelf uses the same corrected key. REJECTED: stamping dateAdded into the acquisition field (corrupts field meaning); live-mirroring desktop's column sort (presentation is per-device — the 7.4.0 presentation-cluster principle — and a KV write per column click).
  3. **Mobile freshness nudge** (Ron, 2026-08-30: "how does the user know to refresh?") — mobile pulls device-state ONCE at load and never re-checks; users learn refresh-to-update only by trial and error. Proposed: on app open / tab-becomes-visible, GET the dstate pointer (a cheap KV READ, not a list — reads are the abundant quota) and compare with the loaded generation → unobtrusive "Newer library available — tap to refresh" banner (mobile twin of desktop's 📡 banner). First layer regardless: show "Library as of ‹time›" somewhere always visible-ish (menu or dashboard footer) so staleness is at least legible. NOT built in alpha.3 — needs relay-client surface + banner UI; follow-on.
  4. **Count honesty on BOTH surfaces** — hidden books make desktop say 3,136 where mobile says 3,134. Fix: "3,134 of 3,136 (2 hidden)" with the hidden clause only when hidden books are among the excluded; tooltip/tap label says **"hidden by user"** (Ron: plain "hidden" just re-poses the mystery — hidden by whom?); on mobile the clause is tappable → flips Show Hidden (the label naming the cause IS the affordance revealing it).
...
- [ ] **Series fetcher gap detection: owned books skipped BEFORE position parsing** (diagnosed 2026-08-29, live case Garrett P.I.) — `hasOwnership` items bail out of parseSeriesBooks (series-page-fetcher.js:358) contributing title-only to skippedOwned, so detectGaps never sees their positions → owned #1 falsely reported "missing from Amazon's series list" AND owned books above the last unowned one silently truncate the detected max (gaps up there can never be reported). Fix: parse position before the ownership skip, carry positions in skippedOwned, feed both sets to detectGaps; also "1 books" → "1 book". Fix agreed in principle, awaiting go; re-running the same series page is the ready-made test (expect: 2 owned, 12 in wishlist, NO missing warning). NOTE: the originally-suspected format-blind duplicate check was EXONERATED — PB and Kindle have different ASINs and both records coexisted correctly; "Already on your wishlist" was literally true (the series run had wishlisted Kindle-#13).

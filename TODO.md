...
- [ ] **10. Mail-run reclamation must RETRY, not fire once** (decided 2026-09-07; prod debris deliberately KEPT as the live test case — do NOT hand-clean first): today `deleteAbsorbedBulkRuns` runs only inside the fire-and-forget consolidation of the import that absorbed a run (js:4845-4857) — tab closes early ⇒ that ~17MB run is never offered for deletion again (later imports skip absorbed runs before download). Ron's prod: 7 absorbed bulk runs ≈124MB idle since Sep-1. Fix: readMailbox's key-listing pass already sees absorbed-but-still-present runs — harvest their runIds + seq counts and attempt deletion of multi-part ones on EVERY import (clearing invariant intact: ledger-absorbed only). Protects rare-use / quick-session users. **Proof**: one prod import ⇒ console `🧹 Reclaimed 7 absorbed bulk run(s)`, keys gone from wrangler listing, ~124MB drop in the next daily summary. (Prod's 3 excess dstate gens need no fix — the next dstate push GCs them; legacy single + everything else TTLs away Nov 26-30 regardless.)

...
- [ ] **Book right-click menu — big-picture reorg** (grown organically; regroup by KIND of action): cluster **placement** — Move to / Copy to / Add to Book List / **Auto-Organize** (it's auto-placement, belongs with them); cluster **metadata edits** — Edit / Tags / Price goal / **Number by current order** (position edit — move it OUT of the Delete neighborhood, it's not destructive); **Share**; then a **divider + Delete** isolated at the bottom. (Ron, 6.14.0 testing.)
- [ ] **Remove "Add Note" from the book context menu** — redundant now that the edit dialog covers it, and a lone Add-Note is inconsistent (why not Edit Title/Position?). Predates the edit dialog. Tags + Price goal keep their spots (frequent quick actions). (Ron.)
- [ ] **Left-pane section-header control order — make consistent** — the **+ (add)** and **collapse/expand toggle** sit in OPPOSITE orders between BOOK LISTS and FOLDERS, training conflicting muscle memory (Ron keeps hitting the wrong one). Fix: anchor the universal **collapse toggle at the far-right** of every section header (only control every section has), with section-specific buttons (+, sort ⇅) to its left in a consistent order. Bigger/ideal alternative: move the collapse chevron to the LEFT of the section label (standard accordion), leaving only action buttons trailing. (Ron, 6.14.0 testing.)

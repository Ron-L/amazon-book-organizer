...
- [x] **Inbox Date Acquired sort incorrect** - ✅ RESOLVED: Fixed (exact alpha version not recorded)
...
- [x] **Column reordering** - ✅ COMPLETE (alpha.172-172.3): Drag column headers to reorder with ghost preview and drop indicator. Global persistence (same order in all folders). Performance optimized: reduced re-renders from 400 to 10 per drag by checking state changes before calling setState.
- [x] **Multi-column sorting** - ✅ COMPLETE (alpha.174-174.4): Shift+Click to add secondary/tertiary sorts (max 3). Visual indicators with subscripts (▲₂ ▼₃). Tooltip for discoverability. Multi-level sort logic loops through array. Manual mode ignores shift (no ties to break). Per-folder persistence with migration.
- [ ] **Search (jump-to)** - ⏸️ DEFERRED: Partial implementation (alpha.175-175.2) on branch feature/search-jump-to. Low value without highlighting; browser Ctrl+F sufficient. May revisit as dropdown navigator in future.
- [x] **Nested folder creation via drag** - ✅ COMPLETE: Drag folder onto another to create as child (exact alpha version not recorded)

### Pending - Other
- [x] **Ctrl+A select all in right pane** - ✅ RESOLVED: Fixed to only select books, not folders (exact alpha version not recorded)
